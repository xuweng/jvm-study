 ...
 * 而是划分为多个(通常是2048个)可以存放对象的 小堆区(smaller heap regions)。
 * 每个小堆区都可能是 Eden区, Survivor区或者Old区. 在逻辑上,
 * 所有的Eden区和Survivor区合起来就是年轻代, 所有的Old区拼在一起那就是老年代:
 * <p>
 * 这样的划分使得 GC不必每次都去收集整个堆空间,
 * 而是以增量的方式来处理: 每次只处理一部分小堆区,称为此次的回收集(collection set).
 * 每次暂停都会收集所有年轻代的小堆区, 但可能只包含一部分老年代小堆区:
 * <p>
 * G1的另一项创新, 是在并发阶段估算每个小堆区存活对象的总数。
 * 用来构建回收集(collection set)的原则是: 垃圾最多的小堆区会被优先收集。这也是G1名称的由来: garbage-first。
 */
public class G1GC {
}
